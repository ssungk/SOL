...
func NewServer(port int, streamConfig RTMPStreamConfig, externalChannel chan<- interface{}, wg *sync.WaitGroup) *Server {
...
// channelHandler 채널 이벤트 처리 (단순화됨 - 세션 종료만 처리)
func (s *Server) channelHandler(data interface{}) {
	switch v := data.(type) {
	case media.SessionTerminated:
		s.handleSessionTerminated(v.SessionId)
	default:
		// 다른 이벤트들은 MediaServer로 직접 전달됨
		s.forwardEvent(data)
	}
}

// handleSessionTerminated 세션 종료 처리
func (s *Server) handleSessionTerminated(sessionId string) {
	// 세션을 직접 찾기 (O(1))
...
// GetSessionById 세션 ID로 세션 찾기 (외부 인터페이스)
func (s *Server) GetSessionById(sessionId string) *session {
	return s.sessions[sessionId]
}

// forwardEvent 이벤트를 외부 채널로 전달
func (s *Server) forwardEvent(event interface{}) {
	select {
	case s.externalChannel <- event:
		// 이벤트 전송 성공
	default:
		// 채널이 꽉 찬 경우 경고 로그
		slog.Warn("External event channel full, dropping event", "eventType", fmt.Sprintf("%T", event))
	}
}

// cleanupSessionFromStreams 세션 정리 (단순화됨 - 실제 스트림 관리는 MediaServer에서 담당)
func (s *Server) cleanupSessionFromStreams(session *session) {
	// 세션에서 소스/싱크 정리는 session.cleanup()에서 이미 수행됨
	// 여기서는 추가 정리 작업이 필요한 경우에만 수행
	slog.Debug("Session cleanup completed", "sessionId", session.GetID())
}
